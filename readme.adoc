...
The `tools/` directory contains shell scripts that predate `sandbox-cli`. They are still functional but `sandbox-cli` is the recommended replacement:
...
| `tools/token-activity.sh` | `sandbox-cli jwt activity <id>`
| `tools/list-shared-clusters.sh` | `sandbox-cli cluster list`
| `tools/get-shared-cluster.sh` | `sandbox-cli cluster get <name>`
| `tools/onboard-ocp-shared-cluster.sh` | `sandbox-cli cluster create <name>`
| `tools/offboard-ocp-shared-cluster.sh` | `sandbox-cli cluster offboard <name>`
...
NOTE: The onboard shell script (`onboard-ocp-shared-cluster.sh`) still provides unique value for initial cluster setup because it automates `oc` commands to create the service account and token on the target cluster. Use it for first-time onboarding, then use `sandbox-cli` for subsequent management.
