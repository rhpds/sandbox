...
| `tools/onboard-ocp-shared-cluster.sh` | `sandbox-cli cluster onboard [name]`
...
NOTE: `sandbox-cli cluster onboard` is the preferred method for initial cluster onboarding. It uses client-go to create the service account and token directly, then registers the cluster with the API. The shell script is kept for environments where the Go binary is not available.
